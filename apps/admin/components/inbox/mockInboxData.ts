// apps/admin/components/inbox/mockInboxData.ts
...
export type MessageRole =
  | 'customer'
  | 'instructor' // message actually sent to customer
  | 'ai_draft'; // suggested draft for instructor
...
function messagesFor(convId: string): MockMessage[] {
  ...
  const texts: Record<string, { role: MessageRole; text: string }[]> = {
    c2: [
      {
        role: 'customer',
        text: 'Hello! Do you have availability for two people next weekend?',
      },
      {
        role: 'ai_draft',
        text: 'We have slots on Saturday and Sunday. How many hours would you like?',
      },
      {
        role: 'customer',
        text: 'Do you have availability next weekend?',
      },
      // This must be instructor -> customer (sent message), not customer
      {
        role: 'instructor',
        text: 'Yes, we have availability. How many hours are you thinking?',
      },
    ],

    // Others can follow same rule: ai_draft is suggestion, instructor is what gets sent
    c1: [
