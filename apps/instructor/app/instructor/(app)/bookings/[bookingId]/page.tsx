...
export default async function BookingDetailPage({ params }: PageProps) {
  ...
  try {
    booking = await fetchInstructorBookingServer(bookingId);
  } catch (err: unknown) {
    ...
    <section>
      ...
      {loadFailed && (
        <p style={{ marginBottom: '1rem', color: '#b91c1c', fontSize: '0.875rem' }}>
          Couldn&apos;t load booking. <Link href={`/instructor/bookings/${bookingId}`} style={{ color: 'rgba(129, 140, 248, 0.95)', textDecoration: 'underline' }}>Refresh the page</Link> to retry.
        </p>
      )}
      {booking && <BookingDetail booking={booking} />}
      {booking && <PaymentLinkButton bookingId={booking.id} />}
      <BookingDecisionTimeline
        key={`${bookingId}-${(booking as { updated_at?: string })?.updated_at ?? ''}`}
        bookingId={bookingId}
      />
    </section>
  );
