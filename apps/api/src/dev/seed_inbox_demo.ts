#!/usr/bin/env tsx
/**
 * Seed Inbox Demo — same behaviour as seed_inbox_customers.ts (English, clean slate).
 * Kept as an alternate entry point; canonical script: seed_inbox_customers.ts.
 *
 * Usage (from repo root):
 *   SEED_INSTRUCTOR_ID=<uuid> pnpm -C apps/api exec tsx src/dev/seed_inbox_demo.ts
 *
 * Output: docs/sim_reports/seed_inbox_demo_<timestamp>.md
 */
...
if (!_rawId || !/^[0-9a-fA-F-]{36}$/.test(_rawId)) {
  console.error(
    '❌  SEED_INSTRUCTOR_ID env var is required (UUID).\n' +
      ...
      'pnpm -C apps/api exec tsx src/dev/seed_inbox_demo.ts'
  );
  process.exit(1);
}
...
// ── Demo personas: one phone per scenario for clean, repeatable demos ────────
interface Persona {
  key: string;
  displayName: string;
  phone: string;
...
const PERSONAS: Persona[] = [
  ...
  { key: 'G', displayName: 'Sara', phone: '+15550000007' }, // same name, different phone = separate conversation
  { key: 'H', displayName: 'Marco', phone: '+15550000008' },
];

// ── Scenarios (all English) ───────────────────────────────────────────────────
...
function deterministicExternalId(scenarioId: string, idx: number): string {
  return `seed_demo_${INSTRUCTOR_ID.slice(0, 8)}_${scenarioId}_${idx}`;
}
    ...
    for (let i = 0; i < sc.messages.length; i++) {
      ...
      try {
        const orchResult = await orchestrateInboundDraft({
          ...
          requestId: `seed_demo_${RUN_TS}_${sc.id}_${i}`,
        });
...
function generateReport(results: ScenarioResult[]): string {
  ...
  lines.push('# Seed Inbox Demo — Report');
  lines.push('');
  ...
  lines.push('## Summary');
  lines.push('');
  lines.push('| Scenario | Label | Customer | Conversation | Prerequisite | Messages | Drafts | Duration |');
  lines.push('|---|---|---|---|---|---|---|---|');
  for (const r of results) {
    ...
    lines.push(
      `| ${r.scenario.id} | ${r.scenario.label} | \`${r.customerId.slice(0, 8)}…\` ` +
        `| \`${r.conversationId.slice(0, 8)}…\` | ${prereq} | ${r.messageIds.length} | ${drafts} | ${r.durationMs}ms |`
    );
  }
  lines.push('');
  lines.push('---');
  ...
  for (const r of results) {
    ...
    if (r.seededBookingId) {
      lines.push(`- **Prerequisite Booking ID:** \`${r.seededBookingId}\``);
    }
    lines.push('');
    lines.push('### Messages');
    lines.push('');
  ...
  lines.push('## Demo phones (one conversation per scenario)');
  lines.push('');
  for (const p of PERSONAS) {
    lines.push(`- **${p.key}** ${p.displayName} — \`${p.phone}\``);
  }
  lines.push('');
  lines.push(`_Generated by seed_inbox_demo.ts — run \`${RUN_TS}\`_`);
  lines.push('');
...
async function main(): Promise<void> {
  console.log('=== FrostDesk Seed Inbox Demo (English, clean) ===');
  ...
  for (const sc of SCENARIOS) {
    ...
    const prereqLabel = sc.prerequisiteBooking
      ...
      : '';
    console.log(`▶ ${sc.id} (${sc.label}) — ${persona.displayName} (${persona.phone})${prereqLabel} ...`);
    ...
    console.log(
      `  ✔ ${sc.id} done (${result.durationMs}ms): ` +
        `${result.messageIds.length} msg, ${drafts} drafts` +
        (errs > 0 ? `, ⚠ ${errs} error(s)` : '')
    );
  ...
  const reportPath = path.join(reportDir, `seed_inbox_demo_${RUN_TS}.md`);
